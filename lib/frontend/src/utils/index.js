...
export const reportRangeFromState = state => {
  const query = parse(state.router.location.query)
...
export const roleFocusFromState = state => ({
  role_focus_id: parse(state.router.location.search).role_focus_id,
})
...
export const getMissingAndOverlyLabelsForARepo = (
  confLabels,
  repositoryLabels
) => {
  ...
  const overlyLabels = repositoryLabels.filter(
    resultLabel =>
...
export const getMissingAndOverlyLabels = state => {
  ...
  return getMissingAndOverlyLabelsForARepo(
    confLabels,
...
export const sortRepos = repositories => {
  repositories.sort(function(a, b) {
    return a.repo_name.toLowerCase() > b.repo_name.toLowerCase()
      ? 1
      : b.repo_name.toLowerCase() > a.repo_name.toLowerCase()
      ? -1
      : 0
  })
  return repositories
...
export const sortRoles = roles => {
  roles.sort(function(a, b) {
    return a.role_name.toLowerCase() > b.role_name.toLowerCase()
      ? 1
      : b.role_name.toLowerCase() > a.role_name.toLowerCase()
      ? -1
      : 0
  })
  return roles
...
export const sortUsers = users => {
  users.sort(function(a, b) {
    return a.user.user_name.toLowerCase() > b.user.user_name.toLowerCase()
      ? 1
      : b.user.user_name.toLowerCase() > a.user.user_name.toLowerCase()
      ? -1
      : 0
  })
  return users
...
export const sortProjects = milestones => {
  milestones.sort(function(a, b) {
    return `${a.repo.toLowerCase()}-${a.due_on}-${a.title}` >
      `${b.repo.toLowerCase()}-${b.due_on}-${b.title}`
      ? 1
      : `${b.repo.toLowerCase()}-${b.due_on}-${b.title}` >
        `${a.repo.toLowerCase()}-${a.due_on}-${a.title}`
      ? -1
      : 0
  })
  return milestones
...
export const roleTypes = [
  { value: 'leadlink', name: 'Premier lien', labels: ['Premier lien'] },
  {
    value: 'elected',
    name: 'Rôles élus',
    labels: ['Facilitateur', 'Second lien', 'Secrétaire'],
  },
  { value: 'assigned', name: 'Rôles désignés', labels: [] },
]

export const daysBeforeExpiration = 10
