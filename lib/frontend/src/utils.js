...
import { addYears, startOfMonth, subMonths } from 'date-fns'
...
export const usersFromState = state => {
  ...
  return {
    assignee: strToList(query.assignee) || [state.user],
...
export const timelineRangeFromState = state => {
  ...
  return {
    start: query.start || monthStart.format('YYYY-MM-DD'),
    stop: query.stop || addYears(monthStart).format('YYYY-MM-DD'),
  }
}
...
export const reportRangeFromState = state => {
  ...
  return {
    start: query.start || monthStart.format('YYYY-MM-DD'),
    stop: query.stop || addYears(monthStart, 1).format('YYYY-MM-DD'),
  }
}
