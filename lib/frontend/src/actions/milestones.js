...
const getLastClosedIssues = (
  ...
  for (let i = 0; i < milestones.length; i++) {
    ...
    try {
      ...
      console.error(e)
      dispatch(setError('Erreur pendant la mise à jour ', 'issues'))
    }
    if (response.status >= 200 && response.status < 300) {
      try {
      ...
      return dispatch(
        setError(`Erreur [${response.status}] ${response.statusText}`, 'issues')
      )
    }
  }
...
export const fetchCircleMilestonesIssuesAndItems = () => async (
  dispatch,
  getState
) => {
  let response
