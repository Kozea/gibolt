...
export const deleteMilestoneCircles = (
  ...
  if (state.meeting.results.report_id) {
    try {
      response = await fetch(
        `api/report_milestones/${state.meeting.results.report_id}/${milestoneNumber}/${repoName}`,
        {
          method: 'DELETE',
          credentials: 'same-origin',
          headers: {
...
export async function getMilestoneAndIssues(
  ...
  try {
    response = await fetch(
      `/api/repos/${circleMilestone.repo_name}/milestones/${circleMilestone.milestone_number}`,
      {
        method: 'GET',
        credentials: 'same-origin',
        headers: {
...
export const closeMilestone = (
  ...
  repoName,
  data,
  target = null // other value: 'circle'
) => async dispatch => {
  let response, json
  if (!target) {
    target = 'timeline'
