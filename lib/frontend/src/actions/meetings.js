...
const formatMeeting = meeting => {
  meeting.attendees = meeting.attendees.map(attendee => attendee.user)
  meeting.actions = meeting.actions.map(action => action.item)
  meeting.indicators = meeting.indicators.map(indicator => indicator.item)
...
export const fetchReport = (search = null) => async (dispatch, getState) => {
  ...
  if (json.objects[0]) {
    meeting = formatMeeting(json.objects[0])
