...
export const fetchCircleAgendaIssues = (circleName, meetingName) => async (
  dispatch,
  getState
) => {
  let response
  let ticketsJson = {}
  const params = `labels=${circleName}&labels=${meetingName}&search=&assignee=&involves=` // eslint-disable-line max-len
  try {
