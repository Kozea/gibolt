...
export const toggleEdition = value => ({
...
export const fetchReport = (search = null) => async (dispatch, getState) => {
  ...
  if (json.objects[0]) {
    try {
      content = JSON.parse(json.objects[0].content)
    } catch (e) {
      // old version of meeting report (text only)
      content = {
        attendees: [],
        actions: [],
        indicators: [],
        projects: [],
        agenda: [],
        content: json.objects[0].content,
      }
    }
    delete json.objects[0].content
    meeting = Object.assign({}, json.objects[0], content)
    dispatch(toggleEdition(true))
    dispatch(setResults(meeting, 'meeting'))
  }
}

export const submitReport = history => async (dispatch, getState) => {
  let response
  let json = {}
  const state = getState()
  const content = {
...
export const updateReport = () => async (dispatch, getState) => {
  let response, json, content, meeting
  const state = getState()
  const newContent = {
    attendees: state.meeting.results.attendees,
    actions: state.meeting.results.actions,
  ...
  try {
    response = await fetch(`/api/reports/${state.meeting.results.report_id}`, {
      method: 'PATCH',
      body: JSON.stringify(newReport),
      credentials: 'same-origin',
      headers: {
  ...
  if (response.status >= 200 && response.status < 300) {
    try {
      ...
      for (let i = 0; i < ticketsJson.objects.length; i++) {
        ticketsJson.objects[i].meetingComment = ''
      }
...
export const fetchMeetingData = params => async (dispatch, getState) => {
  ...
  promises.push(dispatch(toggleEdition(false)))
