...
export const submitReport = (event, history) => async (dispatch, getState) => {
  ...
  let indicateurs = [`### ${event.target.form.childNodes[3]
  ...
  indicateurs = draftToMarkdown(
    convertToRaw(
      stateFromHTML(indicateurs + (event.target.form.childNodes[3].
        childNodes[1].childNodes[i + 1].childNodes[0].innerHTML + ':' +
  ...
  const newReport = {
    ...
    draftToMarkdown(convertToRaw(
      stateFromHTML(event.target.form.childNodes[3].childNodes[0].innerHTML))) +
      draftToMarkdown(convertToRaw(
        stateFromHTML('<br />'))) + indicateurs +
        draftToMarkdown(convertToRaw(
          stateFromHTML('<br />'))) +
