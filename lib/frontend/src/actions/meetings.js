...
export const getElectionData = roles => dispatch => {
  const elections = []
  let endDate, distance
  roles.filter(role => role.role_type === 'elected').map(role =>
    role.role_focuses.map(focus => {
      ...
      if (endDate) {
        ...
        if (
          distance < 0 &&
          !elections.find(selectedRole => selectedRole.role_id === role.role_id)
        ) {
          elections.push(role)
        }
      }
  ...
  return dispatch(updateMeeting(elections, 'elections'))
}
...
export const fetchMeetingData = params => async (dispatch, getState) => {
  ...
  if (meetingType === 'Triage') {
    ...
    if (meetingType === 'Gouvernance') {
      promises.push(dispatch(getElectionData(circle.roles)))
    } else {
      dispatch(updateMeeting([], 'elections'))
    }
