...
import { setError, setResults, stateToParams, unsetLoading } from './index'
...
export const getAndToggleCommentsExpanded = issue => async dispatch => {
  if (issue.comments.length === 0) {
    ...
    try {
      ...
      return dispatch(
        setError('Erreur pendant la récupération des commentaires', 'issueForm')
      )
    }
    if (response.status >= 200 && response.status < 300) {
      try {
...
export const updateATicket = (ticket, values) => async (dispatch, getState) => {
  ...
  try {
    ...
    return dispatch(setErrorIssue('Erreur pendant la mise à jour du ticket'))
  }
  if (response.status >= 200 && response.status < 300) {
    try {
      json = await response.json()
    } catch (e) {
      dispatch(setErrorIssue('La réponse ne contient pas de json'))
      return false
    }
    ...
    dispatch(changeIssue(json.objects, ticket.ticket_id))
    return true
  }
  const jsonMessage = typeof json === 'undefined' ? '' : json.message
  dispatch(
    setErrorIssue(`${response.status}: ${response.statusText} ${jsonMessage}`)
  )
  return false
}
...
export const addOrUpdateComment = (
  ...
  if (commentId) {
    ...
    url = `/api/repos/${ticket.repo_name}/tickets/${ticket.ticket_number}/comments`
    method = 'POST'
  }
  try {
    ...
    return dispatch(
      setError('Erreur pendant la mise à jour du ticket', 'issueForm')
    )
  }
  if (response.status >= 200 && response.status < 300) {
    try {
