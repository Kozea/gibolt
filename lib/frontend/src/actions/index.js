...
export const setModal = (display, creation, data) => ({
  type: 'SET_MODAL',
  display,
  creation,
  data,
  ...
  for (const i in results.missingLabels) {
    if (results.missingLabels.hasOwnProperty(i)) {
      ...
      try {
        ...
        allErrors.push(
          `Erreur pendant la récupération des données du label "${results.missingLabels[i].text}"`
        )
      }
      if (response.status >= 200 && response.status < 300) {
        try {
          ...
          allErrors.push(
            `La réponse ne contient pas de json pour le label "${results.missingLabels[i].text}"`
          )
        }
        ...
        allErrors.push(
          `Erreur [${response.status}] ${response.statusText} pour le label "${results.missingLabels[i].text}"`
        )
      }
    }
  }
  ...
  for (const i in results.overlyLabels) {
    if (results.overlyLabels.hasOwnProperty(i)) {
      ...
      try {
        ...
        allErrors.push(
          `Erreur pendant la suppression du label "${results.missingLabels[i].text}"`
        )
      }
      if (response.status >= 200 && response.status < 300) {
        ...
        allErrors.push(
          `Erreur [${response.status}] ${response.statusText} pour le label "${deleteLabel.label_name}"`
        )
      }
    }
  }
