...
export const fetchIssues = () => async (dispatch, getState) => {
  ...
  if (response.status >= 200 && response.status < 300) {
    try {
      ...
      return dispatch(setResults({ ['issues']: ticketsJson.objects }, 'issues'))
    } catch (e) {
...
export const removeSelectedIssuesPriority = () => (dispatch, getState) => {
  const state = getState()
...
export const incrementSelectedIssuesPriority = () => (dispatch, getState) => {
  const state = getState()
  for (const index in state.issues.results.issues) {
    if (state.issues.results.issues.hasOwnProperty(index)) {
      const issue = state.issues.results.issues[index]
...
export const closeSelectedIssues = () => (dispatch, getState) => {
  const state = getState()
  for (const index in state.issues.results.issues) {
    if (state.issues.results.issues.hasOwnProperty(index)) {
      const issue = state.issues.results.issues[index]
