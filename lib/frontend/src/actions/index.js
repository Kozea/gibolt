  ...
  for (const i in missingLabels) {
    if (missingLabels.hasOwnProperty(i)) {
      ...
      try {
        ...
        return dispatch(
          setError('Erreur pendant la récupération des données du label')
        )
      }
      if (response.status >= 200 && response.status < 300) {
        try {
        ...
        return dispatch(createRepositoryLabels(labelsJson.object))
      }
...
export const deleteLabels = () => (dispatch, getState) => {
  ...
  for (const i in overlyLabels) {
    if (overlyLabels.hasOwnProperty(i)) {
      const deleteLabel = overlyLabels[i]
      fetch(`/api/repos/${repoName}/labels/${deleteLabel.label_name}`, {
        method: 'DELETE',
        credentials: 'same-origin',
        headers: {
          'Content-Type': 'application/json',
        ...
        if (response.status >= 200 && response.status < 300) {
          return dispatch(deleteRepositoryLabels(deleteLabel))
        }
        throw new Error(`[${response.status}] ${response.statusText}`)
      })
    }
