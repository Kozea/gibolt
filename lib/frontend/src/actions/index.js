...
export const deleteRepositoryLabel = deletedLabels => ({
  type: 'DELETE_LABELS',
...
export const deleteLabels = () => (dispatch, getState) => {
  ...
  for (const i in overlyLabels) {
    if (overlyLabels.hasOwnProperty(i)) {
      const deleteLabel = overlyLabels[i].label_name
      fetch(`/api/repos/${repoName}/labels/${deleteLabel}`, {
        method: 'DELETE',
        credentials: 'same-origin',
        headers: {
          'Content-Type': 'application/json',
        ...
        .then(response => response.json())
        .then(() => dispatch(deleteRepositoryLabel(deleteLabel)))
    }
  }
}
