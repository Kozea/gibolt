...
export const createRole = (data, history) => async dispatch => {
  ...
  if (response.status >= 200 && response.status < 300) {
    try {
      ...
      return dispatch(
        setError(
          `La réponse ne contient pas de json pour le cercle "${data.role_name}"`,
          'role'
        )
      )
    }
  ...
  if (response.status >= 200 && response.status < 300) {
    try {
      ...
      for (let i = 0; i < roleJson.objects[0].role_focuses.length; i++) {
        roleJson.objects[0].role_focuses[i].role_focus_users = sortFocusUsers(
          roleJson.objects[0].role_focuses[i].role_focus_users
        )
