import { changeRepository, goBack, setError } from './index'
...
export const setFormParams = params => ({
...
export const submitIssue = (event, history) => async (dispatch, getState) => {
  let response, json
  const state = getState()
  const newIssue = {
  ...
  if (response.status >= 200 && response.status < 300) {
    try {
      ...
      return dispatch(goBack(history))
    } catch (e) {
      console.error(e)
