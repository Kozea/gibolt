...
export const fetchCircle = (param = null, onlyCircle = false) => async (
  ...
  await Promise.all(promises)
    .then(res => {
      ...
      promises.push(getAllUsersInfos(circle.roles, state.users.results))
      Promise.all(promises)
        .then(result => {
          // eslint-disable-next-line prefer-destructuring
          circle = result[1]
          circle.parent_circle_name = result[2].circle_name
          circle.parent_circle_is_active = result[2].is_active
          circle.children_circles = result[3].children_circles
          circle.nb_reports = result[4].nb_reports
          dispatch(setResults(circle, 'circle'))
          return circle
        })
        .catch(err => {
