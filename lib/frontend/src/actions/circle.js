  ...
  if (parentCircleId) {
    ...
    if (response.status >= 200 && response.status < 300) {
      try {
        ...
        throw new Error('La réponse (cercle parent) ne contient pas de json')
      }
    } else {
      const jsonMessage =
...
async function getCircleChildren(param) {
  ...
  try {
    ...
    throw new Error('Erreur pendant la récupération des sous-cercle du cercle')
  }
  if (response.status >= 200 && response.status < 300) {
    try {
      ...
      throw new Error('La réponse (sous-cercles) ne contient pas de json')
    }
  } else {
    const jsonMessage =
    ...
    throw new Error(`${response.status}: ${response.statusText} ${jsonMessage}`)
  }
}
...
async function getCircleNbReports(param) {
  ...
  if (response.status >= 200 && response.status < 300) {
    try {
      ...
      throw new Error('La réponse (nb de rapports) ne contient pas de json')
    }
  } else {
    const jsonMessage =
...
export const fetchCircle = (param = null, onlyCircle = false) => async (
  ...
  if (!onlyCircle) {
    ...
    await Promise.all(promises).then(result => {
      // eslint-disable-next-line prefer-destructuring
      circle = result[3]
      circle.parent_circle_name = result[0].circle_name
      circle.parent_circle_is_active = result[0].is_active
      circle.children_circles = result[1].children_circles
      circle.nb_reports = result[2].nb_reports
    })
  }
