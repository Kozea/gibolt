...
export const fetchCircle = (
  ...
  await Promise.all(promises)
    .then(res => {
      ...
      // add leadlind roles from circle children
      // to display user in attendees list and
      // for circle children leadlink 'election'
      if (circle.circle_children.length > 0) {
        circle.circle_children.map(c => {
          c.roles.filter(role => role.role_type === 'leadlink').map(role => {
            role.role_name = `${role.role_name} [${c.circle_name}]`
