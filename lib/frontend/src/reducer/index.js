    ...
    case 'TOGGLE_COMMENTS_EXPANDED':
      return {
        ...
        results: {
          issues: state.results.issues.map(issue => {
            if (issue.ticket_id === action.issueId) {
              return { ...issue, comments_expanded: !issue.comments_expanded }
            }
            return issue
          }),
        },
...
const organisation = (state = initial.organisation, action) => {
  switch (action.type) {
    case 'CREATE_CIRCLE':
      return {
        results: {
          ...state.results,
          circles: state.results.circles.concat(action.created),
        },
      }
    // case 'DELETE_CIRCLE':
    //   return {
    ...
    //     results: {
    //       ...state.results,
    //       circles: state.results.circles.filter(
    //         circle => action.deleted.circle_id !== circle.circle_id
    //       ),
    //     },
    //   }

    default:
      return handleResultsLoadingAndError(state, 'organisation', action)
  }
}
