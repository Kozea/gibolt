...
const meetingsTypes = (state = initial.meetingsTypes, action) => {
  switch (action.type) {
    case 'SET_MEETINGS_PARAMS':
      return {
        ...state,
        params: action.params,
      }
    default:
      return handleResultsLoadingAndError(state, 'meetingsTypes', action)
  }
}

const meeting = (state = initial.meeting, action) =>
  handleResultsLoadingAndError(state, 'meeting', action)
