  ...
  switch (action.type) {
    case 'EDIT_ITEM':
      return {
        ...
        results: {
          ...state.results,
          actions: [...state.results.actions].map(item =>
            item.item_id === action.itemId
              ? { ...item, editItem: true }
              : { ...item }
          ),
          indicators: [...state.results.indicators].map(item =>
            item.item_id === action.itemId
              ? { ...item, editItem: true }
              : { ...item }
          ),
    ...
    case 'EDIT_ROLE_FOCUS':
      return {
        ...state,
        is_in_edition: action.isInEdition,
      }
