...
import { fetchReport, setMeetingParams } from '../actions/meetings'
...
export default ({ getState, dispatch }) => next => action => {
  ...
  if (action.type === LOCATION_CHANGE) {
    ...
    if (action.payload.pathname === '/') {
    ...
    } else if (action.payload.pathname === '/createrole') {
      if (state.circle.mustLoad) {
        dispatch(setLoading('circle'))
        dispatch(fetchCircle())
      }
    } else if (
      action.payload.pathname === '/meetings' ||
      action.payload.pathname === '/createReport'
    ) {
      dispatch(
        setMeetingParams({
      ...
      if (action.payload.pathname === '/meetings') {
        if (state.labels.mustLoad) {
          dispatch(setLoading('labels'))
          dispatch(fetchResults('labels'))
        }
    ...
    } else if (action.payload.pathname === '/meeting') {
      if (state.users.mustLoad) {
        dispatch(setLoading('users'))
        dispatch(fetchResults('users'))
      }
